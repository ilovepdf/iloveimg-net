...
#warning   RequestHelper sending the "debug" parameter equal true the iLoveImg server won't process the request but it will output the parameters received by the server. You can disable it on Core/RequestHelper.cs file by commenting REMOTE_API_DEBUG_ENABLED directive at top of file.
#endif
            ...
            Exception exception)
        {
            ...
            if (response.StatusCode == HttpStatusCode.BadRequest) // 400 Bad Request
            {
                ...
                if (parsedContent.error.type == EnumExtensions.GetEnumDescription(BadRequestErrors.ProcessingError))
                    return new ProcessingException(responseContent, exception);

                if (parsedContent.error.type == EnumExtensions.GetEnumDescription(BadRequestErrors.DownloadError))
                    return new DownloadException(responseContent, exception);

                if (parsedContent.error.type == EnumExtensions.GetEnumDescription(BadRequestErrors.UploadError))
                    return new UploadException(responseContent, exception);

                if (parsedContent.error.type == EnumExtensions.GetEnumDescription(BadRequestErrors.StartError))
